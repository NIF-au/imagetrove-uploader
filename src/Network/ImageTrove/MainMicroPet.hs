...
module Network.ImageTrove.MainMicroPet where
...
import Network.ImageTrove.MainDicom (is5digits)
...
pUploadOneOptions = CmdUploadOne <$> UploadOneOptions <$> strOption (long "mpet-experiment-dir" <> help "Directory containing micro-pet experiment.")
...
readMicroPetProjectID :: FilePath -> IO (Either String String)
readMicroPetProjectID dir = do
    let f = dir </> "project.txt"
    x <- readFile f

    case lines x of
        [projID] -> return $ Right projID
        _        -> return $ Left $ "Could not read project ID from file: " ++ f

identifyMicroPetExperiment :: String -> String -> String -> String -> [String] -> FilePath -> IO (Either String IdentifiedExperiment)
identifyMicroPetExperiment schemaExperiment institution institutionalDepartmentName institutionAddress operators dir = do
    -- How are micropet project IDs stored? I'll assume that there is a file with the ID in there.
    projectID <- readMicroPetProjectID dir
...
tarMicroPetDirectory :: FilePath -> FilePath -> IO (Either String FilePath)
tarMicroPetDirectory tempDir dir = do
...
mpetToMinc :: FilePath -> FilePath -> IO FilePath
mpetToMinc tempDir dir = do
    let up      = joinPath . reverse . tail . reverse . splitDirectories $ dir -- FIXME Can break.
        title   = last $ splitDirectories dir                                  -- FIXME last is dangerous if empty path
    ...
    -- Assume in micro-pet that we have a bunch of files in a single directory.
    mpetFiles <- (filter $ (".img" `isSuffixOf`)) <$> getDirectoryContents dir

    putStrLn $ "mpetToMinc: found these micro-pet IMG files: " ++ show mpetFiles

    -- FIXME No good error handling here!

    forM_ mpetFiles $ \imgFile -> do
        let cmd     = "upet2mnc"
            mncFile = (reverse . drop 3 . reverse $ imgFile) ++ "mnc"
            args    = [imgFile, mncFile]

        putStrLn $ "Running: " ++ show (cmd, args) ++ " in directory " ++ cwd

        -- Should check output of upet2mnc - how well is it behaved?
...
data MicroPetUploadError = MicroPetTarballError             String
                         | MicroPetIdentifyError            String
                         | MicroPetCreateExperimentError    String
                         | MicroPetCreateDatasetError       String
                         | MicroPetCreateFilesError         [(FilePath, A.Result RestDatasetFile)]
                         | MicroPetCreateProjectGroupError  String
                         | MicroPetNoProjectID              String
                         deriving (Show)

-- Parse the project ID.
stage1 :: FilePath -> ReaderT MyTardisConfig IO (Either MicroPetUploadError String)
stage1 dir = do
    projectID <- liftIO $ readMicroPetProjectID dir

    return $ case projectID of
       Left  pidErr     -> Left  $ MicroPetNoProjectID pidErr
       Right projectID' -> Right $ projectID'

-- Create the project group.
stage2 :: String -> ReaderT MyTardisConfig IO (Either MicroPetUploadError RestGroup)
stage2 projectID' = do
    projectResult <- getOrCreateGroup $ "Project " ++ projectID'
    return $ case projectResult of
        A.Error   projErr       -> Left  $ MicroPetCreateProjectGroupError projErr
        A.Success projResult'   -> Right $ projResult'

-- Make tarball of MicroPet experiment directory.
stage3 :: FilePath -> String -> String -> String -> FilePath -> ReaderT MyTardisConfig IO (Either MicroPetUploadError FilePath)
stage3 tarballTempDir schemaExperiment schemaDataset schemaFile dir = do
    tarball <- liftIO $ tarMicroPetDirectory tarballTempDir dir

    return $ case tarball of
        Left err        -> Left  $ MicroPetTarballError err
        Right tarball'  -> Right $ tarball'

-- Convert to MINC files.
stage3a :: FilePath -> String -> String -> String -> FilePath -> ReaderT MyTardisConfig IO (Either MicroPetUploadError [FilePath])
stage3a mincTempDir schemaExperiment schemaDataset schemaFile dir = do
    mincDir     <- liftIO $ mpetToMinc mincTempDir dir
...
stage4 :: String -> String -> String -> String -> [String] -> FilePath -> ReaderT MyTardisConfig IO (Either MicroPetUploadError IdentifiedExperiment)
stage4 schemaExperiment institution institutionalDepartmentName institutionAddress operators dir = do
    ie <- liftIO $ identifyMicroPetExperiment schemaExperiment institution institutionalDepartmentName institutionAddress operators dir

    return $ case ie of
      Left ieErr    -> Left  $ MicroPetIdentifyError ieErr
      Right ie'     -> Right $ ie'

-- Create the experiment.
stage5 :: IdentifiedExperiment -> ReaderT MyTardisConfig IO (Either MicroPetUploadError RestExperiment)
stage5 ie' = do
    e <- createExperiment ie'
    return $ case e of
        A.Error   eerr  -> Left  $ MicroPetCreateExperimentError eerr
        A.Success e'    -> Right $ e'

-- Identify the dataset.
stage6 :: String -> FilePath -> RestExperiment -> ReaderT MyTardisConfig IO (Either MicroPetUploadError RestDataset)
stage6 schemaDataset dir e' = do
    ...
    return $ case ds of
        A.Error   derr  -> Left  $ MicroPetCreateDatasetError derr
        A.Success ds'   -> Right $ ds'

-- Push all the files (tarball, mincs, thumbnails).
stage7 :: String -> [FilePath] -> RestDataset -> ReaderT MyTardisConfig IO (Either MicroPetUploadError [(FilePath, A.Result RestDatasetFile)])
stage7 schemaFile files ds' = do
    ...
    return $ case fileResults of
        ...
        Left  fileErrors   -> Left  $ MicroPetCreateFilesError fileErrors
...
processMicroPetExperiment' :: (Maybe FilePath, Maybe FilePath, Maybe FilePath, [String], String, String, String, String, String, String, FilePath) -> FilePath -> ReaderT MyTardisConfig IO (Either MicroPetUploadError [(FilePath, A.Result RestDatasetFile)])
processMicroPetExperiment' instrumentConfig dir = do
    ...
    tarballTempDir <- liftIO $ createTempDirectory tmp "micropet_experiment"
    mincTempDir    <- liftIO $ createTempDirectory tmp "micropet_to_minc"
...
imageTroveMain = do
  ...
  where
    opts = info (helper <*> pUploaderOptions ) (fullDesc <> header "imagetrove-micropet-uploader - upload micro-pet data to a MyTARDIS server" )
...
doExperiment iconfig dir topDir subDir processedDir = do
    ...
    x <- processMicroPetExperiment' iconfig dir
    case x of
      Left (MicroPetTarballError             s)            -> liftIO $ putStrLn $ "Error creating tarball: " ++ s
      Left (MicroPetIdentifyError            s)            -> liftIO $ putStrLn $ "Error identifying experiment: " ++ s
      Left (MicroPetCreateExperimentError    s)            -> liftIO $ putStrLn $ "Error creating experiment: " ++ s
      Left (MicroPetCreateDatasetError       s)            -> liftIO $ putStrLn $ "Error creating dataset: " ++ s
      Left (MicroPetCreateFilesError         fileErrors)   -> liftIO $ putStrLn $ "Error creating file(s): " ++ show fileErrors
      Left (MicroPetCreateProjectGroupError  s)            -> liftIO $ putStrLn $ "Error creating project group: " ++ s
      Left (MicroPetNoProjectID s)                         -> liftIO $ putStrLn $ "Error: could not find project ID in subject file: " ++ s
